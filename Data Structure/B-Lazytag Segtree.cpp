...
#define mid ((l + r) >> 1)
  ...
  segtree(int n)
      : sum((n << 2) + 4), add((n << 2) + 4), mul((n << 2) + 4), n(n) {}

  void up(int x, int l, int r) { sum[x] = (sum[lc] + sum[rc]) % p; }
...
int main() {
  ...
  while (m--) {
    cin >> op;
    if (op == 1)
      cin >> x >> y >> k, seg.mulv(1, 1, n, x, y, k);
    else if (op == 2)
      cin >> x >> y >> k, seg.addv(1, 1, n, x, y, k);
    else if (op == 3)
      cin >> x >> y, cout << seg.query(1, 1, n, x, y) << endl;
  }
  return 0;
}