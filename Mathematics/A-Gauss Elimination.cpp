...
int gauss(vector<vector<double>>& M) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    int maxrow = i;
    for (int j = i + 1; j < n; ++j) {
      if (fabs(M[j][i]) > fabs(M[maxrow][i])) maxrow = j;
    }
    // swap the maxrow-th row with the i-th row
    for (int j = 0; j <= n; ++j) swap(M[i][j], M[maxrow][j]);
    // if the maximum is 0, it turns out that the entire row is 0, which is
    // unsolvable.
    if (!M[i][i]) return 0;
  ...
  // regularize
  for (int i = 0; i < n; ++i) M[i][n] /= M[i][i], M[i][i] = 1;
  return 1;
}
