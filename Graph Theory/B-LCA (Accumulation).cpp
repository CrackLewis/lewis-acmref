...
struct graph1 {
   ...
   * Invoke lca_init(int) first, then for each query, invoke lca_query(u,v) for
   * results.
   */

  // base-2 logarithm array
  // if there're multiple graphs, consider modifying Log to be a global array
  // object.
  vector<int> Log;
  // anc: LCA cheatsheet
  // Assumed that N<=2^22=4194304. Might need modification in specific problems.
  const static int lca_maxd = 23;
  vector<int> anc[lca_maxd], dep;
  int lca_inited = 0;
  void lca_init(int rt) {
    ...
    function<void(int, int)> lca_dfs = [&](int u, int fa) {
      ...
      for (int i = 1, _i = Log[dep[u]]; i <= _i; ++i)
