 ...
 *
 * The suffix automaton is a type of automaton that constructs all suffices of a
 * certain string.
 *
...
struct SAM {
  ...
  void insert(const char* str, int off) {
    ...
    // for all p from old-ed to 1, if there is no mp[c][p], then assign it to
    // new-ed
  ...
  long long res = 0;
  void add(int u, int V) { v[++cnt] = V, x[cnt] = al[u], al[u] = cnt; }
  void buildtree() {
