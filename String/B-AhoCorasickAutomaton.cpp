 ...
 * It maintains a trie of all pattern strings where every node has a fail pointer pointing to next matching node position.
 * 
 * Each node in the automaton indicates a prefix of a pattern string.
 * Fail pointer of each node points to the longest suffix which exists in the automaton.
 * If the suffix does not exist, fail pointer points to the root.
 * 
...
struct AhoCorasick
{
  ...
  void build_fail()
  {
    ...
    while(Q.size())
    {
      ...
      for(int i=0;i<_Mp::chr_tot;++i)
      {
        ...
        if(tr[i][u])
          fail[tr[i][u]]=tr[i][fail[u]],Q.push(tr[i][u]);
  ...
  //query(string&) returns the number of patterns existing for at least once in the text string.
